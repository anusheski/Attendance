<?php
    ...
    $loginMessage = "<div class='alert alert-info alert-dismissable'>
                    ...
                    <div class='alert alert-warning alert-dismissable'>
                        <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                        <b>Student?</b> <a href='studentRegister.php'>Click here to create <b>student account</b></a>
                    </div>";
    
    // Insert statement
    $required = array("firstname", "lastname", "email", "department", "hire_date", "birth_date", "gender", "home_address", "phone", "username", "password");            // Array to check, if all the fields are filled
    $error = false;
    ...
    if(isset($_POST['createAccountButton'])){
        ...
        if($error == FALSE){                                            // The data are OK, and can be sent to the DB
            ...
            if($number == 0){               // If user, who is creating account doesn't already exist
                ...
                // Insert additional data into prof_info table
                $query = "INSERT INTO prof_info (user_id, department, hire_date, birth_date, gender, home_address, phone)
                            VALUES('$user_id', '$department', '$hire_date', '$birth_date', '$gender', '$home_address', '$phone')";
                mysqli_query($dbc, $query) or die(mysqli_error($dbc));
                
                $query = "INSERT INTO role (user_id, role)
                            VALUES('$user_id', 'professor')";
